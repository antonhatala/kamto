...
$serviceA = service(1, 'monthly', categoryId: 1, sortOrder: 2);
// Roční služba B (due_month červen): živá buňka jen v červnu.
$serviceB = service(2, 'yearly', dueMonth: 6, sortOrder: 1);
// Služba C: archivovaná, ale MÁ platby dřív v roce (archivace uprostřed roku) -> řádek zůstane.
$serviceC = service(3, 'monthly', isArchived: true, sortOrder: 3);
// Služba D: archivovaná a BEZ plateb v roce -> skrytá.
$serviceD = service(4, 'monthly', isArchived: true, sortOrder: 4);
// Služba E: aktivní, ale vznikla AŽ v roce 2027 (po roce 2026) -> skrytá.
$serviceE = service(5, 'monthly', createdAt: '2027-03-01T00:00:00+01:00', sortOrder: 5);
// Služba F: dangling/NULL kategorie (id 999 v seznamu kategorií neexistuje).
$serviceF = service(6, 'monthly', categoryId: 999, sortOrder: 6);
...
$result = YearHeatmap::build(2026, $today, [$serviceA, $serviceB, $serviceC, $serviceD, $serviceE, $serviceF], $payments, $categories);
...
Assert::count(4, $result->rows); // A, B, C, F

// Řazeno dle sort_order, id -> B(1), A(2), C(3), F(6).
Assert::same([2, 1, 3, 6], $visibleIds);
...
$orphanService = service(7, 'yearly', dueMonth: 6, sortOrder: 1);
