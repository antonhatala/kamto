...
// 003_service_sliding + 004_drop_service_sort_order), druhý běh je no-op.
$path = tempnam(sys_get_temp_dir(), 'kamto-test-');
$db = new PdoSqliteDb($path);
$runner = new MigrationRunner($db, TestDatabase::migrationsDir());

$applied = $runner->run();
Assert::same(['001_init', '002_payment_skipped', '003_service_sliding', '004_drop_service_sort_order'], $applied);
Assert::count(4, $db->fetchAll('SELECT * FROM _migration'));
